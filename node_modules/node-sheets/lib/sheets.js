...
class Sheets {
  ...
  async authorizeJWT(key, scopes = [SCOPE_SPREADSHEETS_READ, SCOPE_DRIVE_READ]) {
    try {
      ...
      this.auth = new _googleapis.google.auth.JWT(key.client_email, null, key.private_key, scopes, null);
      await _q.default.ninvoke(this.auth, 'authorize');
    } catch (err) {
      throw err;
    }
  }
  ...
  async getLastUpdateDate() {
    var drive = _googleapis.google.drive('v3');

    const response = await _q.default.ninvoke(drive.files, 'get', {
      auth: this.auth,
  ...
  async getSheetsNames() {
    ...
    const response = await _q.default.ninvoke(sheets.spreadsheets, 'get', {
      auth: this.auth,
  ...
  async tables(ranges) {
    ...
    if (typeof ranges === 'string') {
      ...
      ranges = [{
        name: ranges
      }]; // NOTE: This version doesn't add range `A:ZZZ`
...
function sheetToTable(sheet) {
  ...
  return {
    ...
    rows: otherRows.map(row => (0, _lodash.default)(headers, (row.values || []).map(value => ({
  ...
  if (value.effectiveFormat.numberFormat) {
    switch (value.effectiveFormat.numberFormat.type) {
      ...
      case 'DATE':
        ...
        return value.effectiveValue && value.effectiveValue.numberValue ? ExcelDateToJSDate(value.effectiveValue.numberValue) : new Date();
    }
  }
...
async function getRanges(auth, spreadsheetId, ranges, fields = 'properties.title,sheets.properties,sheets.data(rowData.values.effectiveValue,rowData.values.formattedValue,rowData.values.effectiveFormat.numberFormat)') {
  var sheets = _googleapis.google.sheets('v4');
